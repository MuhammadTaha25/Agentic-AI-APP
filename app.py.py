...
# # --- 4. Get user inputs from sidebar ---
# def get_user_inputs(companies: dict):
#     st.sidebar.header("Select Companies")
...
#     # Manual ticker input
#     custom_tickers = st.sidebar.text_input("Add custom tickers (comma separated):", key="custom_tickers")
    
#     # Combine selected and custom tickers
#     selected_tickers = [companies[name] for name in st.session_state.selected_companies]
#     custom_tickers_list = [t.strip().upper() for t in custom_tickers.split(",") if t.strip()]
...
#     # Query input in main area
#     st.subheader("Ask a Question")
#     user_query = st.text_input("Enter your question about the selected companies:", key="user_query")
    
#     return ", ".join(all_tickers), user_query

# # --- 5. Fetch agent responses ---
# def fetch_agent_responses(tickers, combined_payload, web_agent, finance_agent):
#     # Web search
#     with st.spinner("🔎 Searching the web..."):
#         web_resp = web_agent.run(f"Explain {combined_payload} with web sources")
...
#     if st.button("Submit Query") or user_query:
#         # Validate inputs
...
#             "role": "user", 
#             "content": f"Tickers: {tickers_input}\n\nQuery: {user_query}"
#         })
        
#         # Combine for consistent payload
#         combined = f"{tickers_input} - {user_query}"
        
#         # Fetch agent responses
#         web_resp, finance_resp = fetch_agent_responses(tickers_input, combined, web_agent, finance_agent)
...
def main():
    ...
    custom_tickers = st.sidebar.text_input("Tickers (comma separated):", 
                                          value=st.session_state.tickers_input,
                                          key="custom_tickers")
    
    # Update tickers input when custom input changes
    if custom_tickers != st.session_state.tickers_input:
        st.session_state.tickers_input = custom_tickers
    ...
    st.subheader("Ask a Question")
    user_query = st.text_input("Enter your question about the selected companies:", 
    ...
    if st.button("Submit Query"):
        ...
        # Fetch agent responses
        web_resp, finance_resp = fetch_agent_responses(all_tickers, combined, web_agent, finance_agent)
